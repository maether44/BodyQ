...
export function calcHealthScore(nutriments = {}) {
    ...
    s -= clamp(nutriments['saturated-fat_100g']) > 10 ? 10 : clamp(nutriments['saturated-fat_100g']) > 5 ? 5 : 0;
    ...
    s += clamp(nutriments['proteins_100g']) > 20 ? 15 : clamp(nutriments['proteins_100g']) > 10 ? 8 : 0;
    return Math.max(0, Math.min(100, Math.round(s)));
}

// Score from raw macro values (for AI photo results)
export function calcHealthScoreFromMacros({ protein = 0, carbs = 0, fat = 0, fiber = 0, calories = 0 }) {
    let s = 50;
