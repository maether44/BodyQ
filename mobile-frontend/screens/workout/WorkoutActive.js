...
import { EXERCISES } from "../../data/mockWorkouts";
...
function getExercise(id) {
  return EXERCISES.find((e) => e.id === id) || { name: id, muscle: "" };
}
...
function RestTimer({ seconds, onSkip }) {
  ...
  const color =
    remaining <= 5 ? "#FF6B6B" : remaining <= 15 ? "#FFB830" : "#34C759";
...
function SetRow({ setNum, repsTarget, done, onDone }) {
  ...
  return (
    <View style={[sr.row, done && sr.rowDone]}>
      ...
      <TouchableOpacity
        style={[sr.doneBtn, done && sr.doneBtnDone]}
        onPress={() => onDone(reps)}
        disabled={done}
      >
...
export default function WorkoutActive({ route, navigation }) {
  ...
  // Default workout if none passed
  const WK = workout || {
  ...
  const handleSetDone = (exIdx, setIdx, reps) => {
    ...
    setRestSecs(ex.restSec || 60);
    setResting(true);
  };

  const allDone = WK.exercises.every((ex, i) =>
    Object.values(setStatus[i]).every((s) => s.done),
  );

  const completedSets = Object.values(setStatus).reduce(
    (acc, ex) => acc + Object.values(ex).filter((s) => s.done).length,
    0,
  );
  ...
  return (
    <View style={styles.root}>
      ...
      {/* Rest timer overlay */}
      {resting && (
        <RestTimer seconds={restSecs} onSkip={() => setResting(false)} />
      )}
            ...
            {WK.exercises.map((exItem, i) => {
              ...
              return (
                <TouchableOpacity
                  ...
                  style={[
                    styles.exTab,
                    currentEx === i && styles.exTabActive,
                ...
                >
                  <Text
                    style={[
                      styles.exTabTxt,
                      currentEx === i && styles.exTabTxtActive,
                    ]}
                  >
                    {exDone ? "✓ " : ""}
                    {getExercise(exItem.id).name.split(" ")[0]}
                  </Text>
                </TouchableOpacity>
              );
            })}
          ...
          <View style={styles.exerciseHeader}>
            <View>
              ...
              <Text style={styles.exerciseMuscle}>
                {exInfo.muscle} · {exInfo.equipment}
              </Text>
            </View>
            <View style={styles.exercisePills}>
              <View style={styles.pill}>
                <Text style={styles.pillTxt}>{ex.sets} sets</Text>
              </View>
              <View style={styles.pill}>
                <Text style={styles.pillTxt}>{ex.reps} reps</Text>
              </View>
              <View style={styles.pill}>
                <Text style={styles.pillTxt}>{ex.restSec}s rest</Text>
              </View>
            </View>
          </View>
        ...
        <View style={styles.navRow}>
          ...
          <TouchableOpacity
            style={[
              styles.navBtn,
              styles.navBtnNext,
              currentEx === WK.exercises.length - 1 && styles.navBtnDisabled,
            ]}
            onPress={() =>
              setCurrentEx((p) => Math.min(WK.exercises.length - 1, p + 1))
            }
