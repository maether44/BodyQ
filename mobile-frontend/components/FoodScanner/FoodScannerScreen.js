 ...
 * Standalone scanner UI. Caller supplies `onLogFood` and closing behavior.
 */
...
export default function FoodScannerScreen({
  ...
  const onBarcodeScanned = useCallback(
    ({ data }) => {
      if (!scanning && !loading && mode === "barcode" && !foodResult) {
        handleBarcode(data);
      }
    },
    [scanning, loading, mode, foodResult, handleBarcode],
  );
  ...
  const handleLog = async (mealType = "snack") => {
    ...
    const ok = (await onLogFood?.({ ...foodResult, mealType })) ?? true;
      ...
      <View style={[s.root, { alignItems: "center", justifyContent: "center" }]}>
        ...
        <View style={s.permCard}>
          ...
          <Text style={s.permSub}>
            BodyQ needs your camera to scan barcodes and analyse food photos.
          </Text>
  ...
  return (
    <GestureHandlerRootView style={s.root}>
      ...
      <CameraView
        ...
        facing="back"
        onBarcodeScanned={
          mode === "barcode" && !foodResult ? onBarcodeScanned : undefined
        }
      ...
      <View style={s.scanArea}>
        <ScanFrame active={mode === "barcode" && !loading && !foodResult} />
      </View>
