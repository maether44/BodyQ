...
export default function AppTour({ activeTab, onTabPress, showOnMount = false }) {
  const {
    visible, computed, stepIdx,
    overlayOp, sL, sT, sW, sH, sR,
    bubOp, bubTY, glow,
    next, prev, done,
      ...
      <Animated.View style={[StyleSheet.absoluteFill, { opacity: overlayOp }]}>
        ...
        {computed.hasSpot ? (
          <>
            ...
            <Animated.View style={[s.mask, { top: Animated.add(sT, sH), left: 0, right: 0, bottom: 0 }]} />
            <Animated.View style={[s.mask, { top: sT, left: 0, width: sL, height: sH }]} />
            <Animated.View style={[s.mask, { top: sT, left: Animated.add(sL, sW), right: 0, height: sH }]} />

            <Animated.View style={[s.spotRing, { left: sL, top: sT, width: sW, height: sH, borderRadius: sR, borderColor: glowColor }]} />
        ...
        <Animated.View style={[
          ...
          {computed.arrowSide === 'bottom' && <View style={[s.arrowDown,  { left: computed.arrowOffset }]} />}
          {computed.arrowSide === 'right'  && <View style={[s.arrowRight, { top:  computed.arrowOffset }]} />}

          <View style={s.bubbleCard}>
            <View style={s.row}>
              <View style={s.badge}><Text style={{ fontSize: 15 }}>{step.emoji}</Text></View>
          ...
          {computed.arrowSide === 'top'  && <View style={[s.arrowUp,   { left: computed.arrowOffset }]} />}
          {computed.arrowSide === 'left' && <View style={[s.arrowLeft, { top:  computed.arrowOffset }]} />}
        </Animated.View>
...
const s = StyleSheet.create({
  ...
  spotRing: { position: 'absolute', borderWidth: 2, shadowColor: '#7B61FF', shadowOffset: { width: 0, height: 0 }, shadowOpacity: 0.9, shadowRadius: 14, elevation: 12 },
  ...
  bubbleCard:    { backgroundColor: '#16132B', borderRadius: 20, padding: 16, borderWidth: 1, borderColor: '#2D2850', shadowColor: '#000', shadowOffset: { width: 0, height: 10 }, shadowOpacity: 0.55, shadowRadius: 24, elevation: 24 },
  arrowUp:    { position: 'absolute', top: -ARROW,    width: 0, height: 0, borderLeftWidth: ARROW, borderLeftColor: 'transparent', borderRightWidth: ARROW, borderRightColor: 'transparent', borderBottomWidth: ARROW, borderBottomColor: '#16132B' },
  arrowDown:  { position: 'absolute', bottom: -ARROW, width: 0, height: 0, borderLeftWidth: ARROW, borderLeftColor: 'transparent', borderRightWidth: ARROW, borderRightColor: 'transparent', borderTopWidth: ARROW, borderTopColor: '#16132B' },
  arrowLeft:  { position: 'absolute', left: -ARROW,   width: 0, height: 0, borderTopWidth: ARROW, borderTopColor: 'transparent', borderBottomWidth: ARROW, borderBottomColor: 'transparent', borderRightWidth: ARROW, borderRightColor: '#16132B' },
  arrowRight: { position: 'absolute', right: -ARROW,  width: 0, height: 0, borderTopWidth: ARROW, borderTopColor: 'transparent', borderBottomWidth: ARROW, borderBottomColor: 'transparent', borderLeftWidth: ARROW, borderLeftColor: '#16132B' },
